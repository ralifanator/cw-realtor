...
while n < 500:
    ...
    addr = Json({'district' : next(districts), 'street' : next(streets), 'home' : random.randrange(1, 120), 'flat' : random.randrange(1, 200)})
    passport = Json({'series' : ''.join(next(ser)), 'num' : ''.join(next(num)), 'date' : randomDate("1/1/2008", "9/1/2023", random.random())})
    cursor.execute("INSERT INTO clients (full_name, phone_number, e_mail, passport_data, address) VALUES (%s, %s, %s, %s, %s)", (name, phone, mail, passport, addr))
