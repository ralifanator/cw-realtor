...
while n < 500:
    ...
    preferences = Json({'district' : random.choice(districts), 'street' : random.choice(streets), 'roof' : random.choice(roofs), 'deal_type' : random.choice(deal_types), 'cost' : random.randrange(5, 5000),'other' : 'хочу подешевле' })
    cursor.execute("INSERT INTO clientsrequests (id_client, type_immovable, preferences) VALUES (%s, %s, %s)", (id_client, type_immovable, preferences))
    id_clients.remove(id_client)
