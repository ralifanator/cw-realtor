...
while n < 500:
    ...
    id_type = random.randrange(1, 7)
    address = Json({'district' : random.choice(districts), 'street' : random.choice(streets), 'home' : random.randrange(1, 120), 'flat' : random.randrange(1, 200)})
    parameters = Json({'deal_type' : random.choice(deal_types), 'cost' : random.randrange(1, 5000), 'num_of_rooms' : random.randrange(1, 7), 'other' : 'в хорошем состоянии'})
    desc = 'неплохой вариант для студентов, готов к торгу'
    cursor.execute("INSERT INTO immovables (id_type, address, parameters, description, id_owner) VALUES (%s, %s, %s, %s, %s)", (id_type, address, parameters, desc, id_owner))
